...
import {
  MAP_W, MAP_H, SCREENS, BUILDINGS, DECOS, NPC_CHATS,
  CITIZENS as CITIZENS_DEF, COURT_W, COURT_H, JURY_LEFT, JURY_CENTER, JURY_RIGHT, JURY_AREAS,
  DEFENDANT_SEAT, COURT_NPCS as COURT_NPCS_DEF, JURY_NPCS as JURY_NPCS_DEF, PHASES,
  SHOP, BATH_ITEMS, CHATS, GUILTY_ZONE, NOTGUILTY_ZONE, CENTER_ZONE, EXIT_POS
  ...
  useEffect(() => {
    ...
    const unsub = subscribeAuth((u) => {
      ...
      if (u) {
        ...
        // 로그인했던 사용자가 로그아웃하면 랜딩페이지로 복귀
        // (한 번도 로그인하지 않은 오프라인 플레이어는 그대로 유지)
        if (hadUser.current) {
    ...
    <>
      ...
      <div id="bath-screen" className="screen">
        <div className="bath-container">
          ...
          <p className="shop-info">금화: <b id="bath-coins">0</b> 💰</p>
...
function initGame() {
  ...
  const def = () => ({
    name: "", head: "😊", body: "#3498db", eggs: 10, tomatoes: 10,
  ...
  /* 게임 상수(MAP_W/MAP_H, BUILDINGS, DECOS, NPC_CHATS, CITIZENS, 재판소 좌표,
     COURT_NPCS, JURY_NPCS, PHASES 등)는 lib/game/constants.js 에서 import 합니다. */

  /* 런타임에 위치·얼룩·타이머가 변형되는 배열은 모듈 상수를 직접 바꾸지 않도록
     게임 시작 시 새 복제본을 만든다 (리마운트 시 항상 초기 상태로 시작) */
  const CITIZENS = CITIZENS_DEF.map((c) => ({ ...c }));
  const COURT_NPCS = COURT_NPCS_DEF.map((c) => ({ ...c, range: c.range ? [...c.range] : c.range }));
  const JURY_NPCS = JURY_NPCS_DEF.map((c) => ({ ...c }));
  /* CITIZENS 런타임 속성 초기화 (이동 목표/대화 타이머) */
  ...
  const P = {
    x: S.x || 800, y: S.y || 480, r: 18, speed: 4.2, vx: 0, vy: 0,
  ...
  /* 플레이어가 장애물(건물/분수) 안에 있는지 판정 */
  const playerBlockedAt = (px, py) => BUILDINGS.some((b) => {
  ...
  /* 스폰 위치가 장애물 안(예: 광장 중앙 분수)이면 안전한 위치로 이동시켜 갇힘 방지 */
  function unstickPlayer() {
    if (!playerBlockedAt(P.x, P.y)) return;
    const safeSpots = [[800, 480], [800, 860], [520, 620], [1080, 620], [800, 300], [400, 900], [1200, 900]];
    for (const [sx, sy] of safeSpots) {
      if (!playerBlockedAt(sx, sy)) { P.x = sx; P.y = sy; return; }
    }
  }
  ...
  function updateTown() {
    ...
    /* 이미 장애물 안(예: 분수 중앙)에 갇힌 경우 빠져나올 수 있도록 이동 허용 */
    const stuckNow = blocked(P.x, P.y);
    if (stuckNow || !blocked(nx, P.y)) P.x = nx;
    if (stuckNow || !blocked(P.x, ny)) P.y = ny;
    ...
    for (let i = projs.length - 1; i >= 0; i--) {
      ...
      for (const c of CITIZENS) {
        if (Math.hypot(p.x - c.x, p.y - c.y) < P.r + 8) {
          ...
          c.hitLock = 20;
          if (p.kind === "tomato") { c.tomatoStain = Math.min(20, (c.tomatoStain || 0) + 1); c.lastStainTime = Date.now(); c.stainStageApplied = 0; playSound("splat"); splat.push({ x: c.x, y: c.y, emoji: "💥", t: 25 }); S.personality = Math.max(-9999, S.personality - 5); toast("🍅 명중! 인성 -5"); updateHUD(); }
          else if (p.kind === "egg") { c.eggStain = Math.min(20, (c.eggStain || 0) + 1); c.lastStainTime = Date.now(); c.stainStageApplied = 0; playSound("splat_egg"); splat.push({ x: c.x, y: c.y, emoji: "💥", t: 25 }); S.personality = Math.max(-9999, S.personality - 10); toast("🥚 명중! 인성 -10"); updateHUD(); }
  ...
  /* 건물별 출구 위치(EXIT_POS)는 lib/game/constants.js 에서 import */
  function exitCourt() {
    /* 요약 오버레이가 이미 열려 있으면 중복 생성 방지 */
    if (document.getElementById("court-exit-ok")) return;
  ...
  if (btnLogin) btnLogin.addEventListener("click", async () => {
    ...
    try {
      ...
      if (S.name) {
        ...
        P.name = S.name; P.x = S.x || 800; P.y = S.y || 480; unstickPlayer();
  ...
  const onFirebaseLogin = async (e) => {
    currentUser = e.detail;
    await loadUserData(currentUser.uid);
    save();
    // 랜딩페이지에서 로그인 후 진입한 경우: 로그인 화면이면 자동으로 다음 단계로 진행
    ...
    if (loginScr && loginScr.classList.contains("active")) {
      if (S.name) {
        ...
        const emailName = (currentUser.email || "").split("@")[0].slice(0, 8);
        const nameInp = document.getElementById("char-name");
        if (nameInp && !nameInp.value) nameInp.value = emailName;
        drawCharPreview(); showScreen("char-screen");
      }
    }
  };
  ...
  window.addEventListener("firebase-login", onFirebaseLogin);
  window.addEventListener("firebase-logout", onFirebaseLogout);
  ...
  async function advancePhase() {
    ...
    if (currentPhase.name === "verdict") {
      ...
      S.stats.courts++; save();
      /* 재판 결과 요약 모달은 throw 단계 종료 후 표시 (투척 중 알림 차단 문제 수정) */
    ...
    if (currentPhase.name === "throw") {
      ...
      /* 재판 결과 요약 모달 (throw 종료 후 표시 - 투척 중 알림 차단 문제 수정) */
      const csThrow = document.getElementById("court-screen");
      const inCourtThrow = csThrow && csThrow.classList.contains("active");
      if (inCourtThrow) {
  ...
  function startNewCourtTrial() {
    ...
    if (S.registeredStories && S.registeredStories.length > 0) {
      ...
      /* AI가 학습하여 비슷한 사연 생성 */
      const genStory = generateRandomStory();
      ct.story = genStory.content;
      ct.storyTitle = genStory.title;
      ct.storyDefendant = genStory.defendant;
      defendantChar = { x: DEFENDANT_SEAT.x, y: DEFENDANT_SEAT.y, r: DEFENDANT_SEAT.r, head: randHead, body: randColor, name: genStory.defendant || "피고인", gold: 0 };
    }
    const storyEl = document.getElementById("ct-story");
    if (storyEl) storyEl.textContent = "";
  }
  ...
  /* 배심원석 위치(GUILTY_ZONE/NOTGUILTY_ZONE/CENTER_ZONE)는 lib/game/constants.js 에서 import */
  ...
  /* 정화소 아이템(BATH_ITEMS)은 lib/game/constants.js 에서 import */
  function openBath() {
    const g = document.getElementById("bath-grid"); if (!g) return;
    g.innerHTML = "";
    document.getElementById("bath-coins").textContent = S.gold;
  ...
  /* 언마운트 시 정리: 렌더 루프/전역 리스너/타이머를 모두 해제해
     리마운트(/game→/→/game, StrictMode) 시 중복 실행을 방지한다 */
  return function teardownGame() {
    if (rafId) cancelAnimationFrame(rafId);
    clearInterval(bgmTimer); clearInterval(confInt); clearInterval(adInt); clearInterval(courtTimer);
    document.removeEventListener("click", ensureAudioReady);
    ...
    window.removeEventListener("firebase-login", onFirebaseLogin);
    window.removeEventListener("firebase-logout", onFirebaseLogout);
  };
}